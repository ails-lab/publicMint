			...
			startup : function(dataFromURL){
				...
				if (dataFromURL && ('kConnector' in dataFromURL))
				{
					...
					$navBlock.append(kTemplater.jQuery('line.summary', {
						label	: 'MINT FOR ALL',
						info	: 'The MINT service facilitates upload of arbitrary record shaped data in JSON, CSV or XML; mapping into a defined xml-schema; transformation of the data into that XML and export to various targets. ',
				...
				<%if(user.hasRight(User.MODIFY_DATA) || user.hasRight(User.ADMIN)){%> 						
					$navBlock.append(kTemplater.jQuery('line.navigation', {
						label : 'Locks',
						data : { kConnector:"html.page", url:"html/locks.html", kTitle:"Locks" }
					}));
                <%}%>                
...
<script>
	$(document).ready(function () {
		...
		var deepLink = <s:property value="deepLink" />;
