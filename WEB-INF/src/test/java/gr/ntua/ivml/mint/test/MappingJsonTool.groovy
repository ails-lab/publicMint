...
cli.pass( 'password if diffferent from user', args:1 )
...
try {
	...
	String password = options.u
        if( options.pass ) password = options.pass 
	def sql =  Sql.newInstance( url, options.u, password, "org.postgresql.Driver" )
