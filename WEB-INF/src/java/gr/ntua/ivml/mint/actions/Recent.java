...
public class Recent extends GeneralAction {
	...
	public String execute() {
		...
		else if(this.getType().equals("mappingbyorg")) {
			this.doMappings(organization,dataset);
		}
	...
	public void doMappings() {
		...
		try {
			...
			User u = this.getUser();
			if( u.getRights() > 0 ) {
				List<Mapping> list = DB.getMappingDAO().findByOrganization(u.getOrganization());
					...
					while(i.hasNext()) {
						...
						if(recent.containsKey("dataset")) {
							try {
								...
								if( dataset.getOrganization().getDbID() == u.getOrganization().getDbID()) {
	...
	public void doMappings(long org,long ds) {
		json = new JSONObject();
		
		try {
			JSONArray mappings = new JSONArray();
			
			List<Mapping> list = DB.getMappingDAO().findByOrganization(DB.getOrganizationDAO().findById(org, false));
			for(Mapping mapping: list) {
				JSONArray recents = Meta.getArray(mapping, Mapping.META_RECENT_DATASETS);
				Iterator<?> i = recents.iterator();
				...
				while(i.hasNext()) {
					...
					if(recent.containsKey("dataset")) {
						try {
							if (Long.parseLong(recent.get("dataset").toString())==ds){
								Dataset dataset = DB.getDatasetDAO().findById(ds, false);
	...
	public void setRecent(String type, long org,long ds) {
		this.type = type;
