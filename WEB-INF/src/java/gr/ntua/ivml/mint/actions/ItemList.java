...
//				}
		...
		if( getUser().hasRight(User.SUPER_USER)) allow=true;
			...
			if( owner == null ) {
				...
				if( getUser().can("view data", owner ) ||
						getUser().sharesProject(result.getOrigin())) allow = true;
			}
		}
	...
	/**
	 * Find the right page for the given itemId and return it.
	 * If it can't find it, empty list and page 0 is probably as good as any a return.
	 * @return
	 */
	private List<Item> readItemPage( ) {
		List<Item> values = null;
		List<Long> ids = DB.getItemDAO().getItemIdsByDataset(getDataset());
		int page = 0;
		int idx = 0;
		int resIdx = -1;
		for( Long itemIdDb: ids ) {
			if( itemIdDb == itemId ) {
				resIdx = idx;
				break;
			}
		...
		if( resIdx >= 0 ) {
			values = DB.getItemDAO().getItemsByDataset(getDataset(), page*getMax(), getMax());
			json.element( "page", page);
			json.element( "idxOnPage", resIdx);
		}
		return values;
	}
	...
	private List<Item> solrQueryItems() {
		...
		if( StringUtils.empty( queryType ) || queryType.equals( "label") ) itemIds = solrQueryTitle(sq);
			...
			itemIds = solrQueryAll(sq);
		else if (queryType.equals("field")) {
			itemIds = solrXpathQuery(sq);
		}
	...
	private List<Long> solrXpathQuery(SolrQuery sq) {
