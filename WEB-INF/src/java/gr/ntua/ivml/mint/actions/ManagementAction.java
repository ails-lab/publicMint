...
@Results({
	  ...
	  @Result(name="rootaccess", location="uaccessibility.jsp")
	})
	   ...
	   public String execute() throws Exception {
		   try{
			   if(getUaction().equalsIgnoreCase("showuser") || getUaction().equalsIgnoreCase("edituser")){
				   ...
				   if( user.getProjectIds().size() == 1 ) {
					   Integer adminProjectId = user.getProjectIds().get(0);
					   if( ! selorg.getProjectIds().contains(adminProjectId))
						   selorg.getProjectIds().add( adminProjectId );
				   }
			   ...
			   else if(getUaction().equalsIgnoreCase("accessibility")) {
			   ...
			   else if(getUaction().equalsIgnoreCase("saveaccessibility")) {
				   log.debug("saveaccessibility UAction");
				   if(getPublish() != null && getPublish()) {
					   log.debug("Only root can publish");
                       Config.setLive("disablePublish", "true");
				   } else if(getPublish() == null || !getPublish()) {
					   Config.setLive("disablePublish", "false");
				   }
				   if(getLogin() != null && getLogin()) {
					   log.debug("Only root can login");
                       Config.setLive("disableLogin", "true");
				   } else if(getLogin() == null || !getLogin()) {
					   Config.setLive("disableLogin", "false");
				   }
				   return "rootaccess";
			   }
		   }catch(Exception ex){
