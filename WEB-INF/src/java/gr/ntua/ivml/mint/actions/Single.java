...
public class Single extends GeneralAction  {
	...
	private String composeHomeUrl() {
		...
		if(parameters.containsKey("action")) {
			String action = parameters.get("action").getMultipleValues()[0];
			String kTitle = null;
			String query = null;
			...
			for(String key: parameters.keySet()) {
				if(!key.equals("action")) {
					if(key.equals("kTitle")) {
						kTitle = parameters.get("kTitle").getMultipleValues()[0];
					} else {
						if(query == null) {
							query = "?";
						} else {
						...
						String parameter = parameters.get(key).getMultipleValues()[0];
						query += key + "=" + parameter;
					}
				}
			}
