...
//	}
	...
	public  JSONObject listTransformations() {
		...
		if (this.organizationId == null){
			res = DB.getDataUploadDAO().pageAll(Order.desc("created"), start, maxResults);
		}
		else{
	...
	public JSONObject listDatasets() {
		List<Dataset> res;
		if (this.organizationId == null){
			res = DB.getDatasetDAO().pageAll(Order.desc("created"), start, maxResults);
		}
		else{
	...
	public void listMappings( boolean rdf ) {
		...
		if(getUser().hasRight(User.SUPER_USER)) {
			res = DB.getMappingDAO().pageAll( Order.desc( "creationDate"),start, maxResults );
		} else {
	...
	public JSONObject listUsers( boolean rdf ) {
		List<User> res = DB.getUserDAO().pageAll( Order.desc( "dbID"), start, maxResults );
	...
	public JSONObject listOrganizations() {
		...
		JSONArray arr = new JSONArray();
		if (this.organizationId == null){
		 res = DB.getOrganizationDAO().pageAll( Order.desc("dbID"),start, maxResults );
...
//		List resr  = DB.getPublicationRecordDAO().pageAll(start, maxResults);
//		JSONArray arr = new JSONArray();
//		for( DataUpload du: res ){
//			if (du)
...
//	}
	...
	/*public JSONObject listPublications() {
		...
		for( PublicationRecord du: resr ){
			if (du.getPublishedDataset().isOk()){
				if ( this.organizationId==null || du.getOrganization().getDbID()==Long.parseLong(organizationId)){				
					arr.add( du.getPublishedDataset().toJSON());
				}
			}
		}
	...
	public JSONObject listPublications() {
		...
		if (this.organizationId != null){
			...
			resr  = DB.getPublicationRecordDAO().pageAll(Order.desc("startDate"),start, maxResults);			
		}
		JSONArray arr = new JSONArray();
		for( PublicationRecord pr: resr ){
			...
			if (pr.getPublishedDataset().isOk()){
					...
					if (pr.getOriginalDataset().getSchema()!=null) jsob.put("targetSchema",pr.getOriginalDataset().getSchema().getName());
					 					
					//jsob.put("publishedItems", pr.getPublishedItemCount());
					jsob.put("name",pr.getOriginalDataset().getName());
					if (pr.getOriginalDataset() instanceof DataUpload) jsob.put("parentDataset",pr.getOriginalDataset().getName());
					
					if  ((Config.get("custom.name")!=null)&&(!Config.get("custom.name").equals("euscreenxl"))){
						jsob.put("publishedItems", pr.getPublishedDataset().getValidItemCount());
					}
					else jsob.put("publishedItems", pr.getPublishedItemCount());
					//jsob.put("publishedItems", pr.getPublishedDataset().getValidItemCount());
