...
@Results({
	  ...
	  @Result(name="enrichment_input", location="enrichment_import.jsp")
	})
...
public class Import extends GeneralAction  {
    ...
    public String execute() throws Exception {
    	...
    	if( !user.can( "change data", du.getOrganization() )) {
    		List<Integer> projectIds = Project.sharedIds( 
    				user.getProjectIds() ,
    				du.getOrganization().getProjectIds()
    				);
    		if( projectIds.isEmpty() ) {
    			throw new IllegalAccessException("Parameter manipulation");
    		}
    		
    		du.setProjectIds(projectIds);
    	}
		...
		// should be null anyway, but UI allows for schema even on csv upload (TBFixed)
		if( this.isCsv ) upI.getDataUpload().setSchema( null );
		
		if( upI != null && getIsJson()) {
			upI.json = true;
		}
		
		if( upI != null && getIsMarc()) {
			upI.marcPreprocess = true;
		}
	...
	@Action("Import_input_enrichment")
	public String input_enrichment() throws Exception {
		if( user.getOrganization() == null && !user.hasRight(User.SUPER_USER)) {
			throw new IllegalAccessException( "No import rights!" );
		}
	...
	public void setIsMarc(Boolean isMarc) {
		this.isMarc = isMarc;
	}
