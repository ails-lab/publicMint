...
public class Transform extends GeneralAction  {
	...
	public String execute() throws Exception {
		...
		if(this.getSelectedMapping()>0){
			...
			try{
				...
				//Possible AnnotatedDatasets derived from Transformation will be deleted 
				//because of cascade declaration in respective database table
				List<Transformation> lt = du.getTransformations();
				for( Transformation t: lt ){
	...
	public List<Mapping> getRecentMappings() {
		List<Mapping> maplist = getUser().getAccessibleMappings( false );
		if(this.getUploadId() > 0) {
			Dataset dataset = DB.getDatasetDAO().findById(this.getUploadId(), false);
			// filter for this dataset
			maplist = Mapping.getRecentMappings(dataset, maplist);
		} else {
