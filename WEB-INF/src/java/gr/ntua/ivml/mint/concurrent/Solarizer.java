...
public class Solarizer implements Runnable {
	...
	public void runInThread() {
		try {
			...
			extraItemData.put( "organization_id", new String[]{ Long.toString( ds.getOrganization().getDbID()) });
			extraItemData.put( "dataset_id", new String[]{ ds.getDbID().toString() });
			extraItemData.put( "user_id", new String[]{ ds.getCreator().getDbID().toString() });
			extraItemData.put( "project_name_s", ds.getProjectNames());
			
			if( ds.getSchema() != null ) { 
				extraItemData.put("schema_id", new String[]{ ds.getSchema().getDbID().toString() });
				extraItemData.put("schema_name_s", new String[]{ ds.getSchema().getName() });
			}

			PublicationRecord pr = ds.getPublicationRecord();
			if(( pr != null) && ( PublicationRecord.PUBLICATION_OK.equals( pr.getStatus())))
				extraItemData.put( "published_b", new String[]{ "true" });
	...
	public void solarizeStatelessItem( Item item, Map<String, String[]> extra ) throws Exception {
		...
		for( Map.Entry<String,String[]> e: extra.entrySet()) {
			for( String val: e.getValue() ) 
				currentSid.addField(e.getKey(), val);
		}
