...
public class Queues {
	...
	static public Map<String, ExecutorService> queues = new HashMap<String,ExecutorService>();
	...
	static {
		...
		int dbThreads = Integer.parseInt(Config.get("mint.dbThreads", "2"));
		ExecutorService e = Executors.newFixedThreadPool(dbThreads);
		queues.put( "db", e);
		// net queue allows for parallel network heavy operations
		// upload part one goes here (with blob to db part)
		int netThreads = Integer.parseInt(Config.get("mint.netThreads", "4"));
		e = Executors.newFixedThreadPool(netThreads);
		queues.put( "net", e);
