...
public class StringUtils {
	...
	// returns the zipped bytes or empty if things went wrong (which they cant really)
	public static Optional<byte[]> gzipStringUtf8( String input )  {
	...
	// return the unzipped String or empty if things go wrong
	public static Optional<String> ungzipUtf8Bytes( byte[] input ) {
		try (
			ByteArrayInputStream bis = new ByteArrayInputStream(input);
			GzipCompressorInputStream gis = new GzipCompressorInputStream(bis); 
			InputStreamReader isr = new InputStreamReader(gis, "UTF8") ) {
				return Optional.of( IOUtils.toString(isr));
		} catch( Exception e ) {
			log.error( "",  e );
		}
		return Optional.empty();
