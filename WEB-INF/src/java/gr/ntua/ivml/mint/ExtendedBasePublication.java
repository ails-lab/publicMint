...
//	}
	...
	public boolean unpublish(Dataset ds) {
		...
		if (derivedItemsDataset != null) {
			DB.getPublicationRecordDAO().getByPublishedDataset(derivedItemsDataset)
			 .ifPresent( pubRec -> DB.getPublicationRecordDAO().makeTransient( pubRec ));
		}
		if (ExtraItemsDataset != null) {
			DB.getPublicationRecordDAO().getByPublishedDataset(ExtraItemsDataset)
			 .ifPresent( pubRec -> DB.getPublicationRecordDAO().makeTransient( pubRec ));
		}
		if (ExtraItemsDataset2 != null) {
			DB.getPublicationRecordDAO().getByPublishedDataset(ExtraItemsDataset2)
			 .ifPresent( pubRec -> DB.getPublicationRecordDAO().makeTransient( pubRec ));
		}
	...
	//	pr = DB.getPublicationRecordDAO().getByPublishedDataset(originalDataset);
		pr = DB.getPublicationRecordDAO().getByPublishedDataset(theds).orElseGet(
				()-> {
					...
					pubRec.setPublishedDataset(theds );
					pubRec.setOrganization(theds.getOrganization());
					return pubRec;
				});
