...
public class Element extends JSONMappingHandler {
	...
	public ArrayList<SimpleMapping> getAllChildrenMappings() {
		ArrayList<SimpleMapping> mappings = new ArrayList<SimpleMapping>();
		if(!getMappingCases().isEmpty()) {
			for(MappingCase mc : getMappingCases()) 
				mappings.addAll(mc.getMappings());
		}
		for(Element c: this.getChildrenAndAttributes()) {
			mappings.addAll(c.getAllChildrenMappings());
		}
		return mappings;
	}
	...
	public Boolean isSimpleContentType() {
		if (this.has("simpleContentType")) 
			return Boolean.valueOf(this.getString("simpleContentType"));
		else 
			return false;
	}
