...
public class RepositoryValidator {
	...
	protected final static Logger log = Logger.getLogger(RepositoryValidator.class);
	...
	public static boolean isValid(String baseURL){
		...
		if(ident != null){
			...
			try {
				...
				log.error( "OAI Identify answer invalid", e );
				res = false;
			}
		}
	...
	public static ArrayList<String> getNameSpaces(String baseURL){
		...
		try {
			...
			log.error( "Namespaces not retrieved", e );
		} 
		return res;
	}
	...
	public static HashMap<String, String> getSets(String baseURL){
		...
		try {
			...
			if( (errors != null) && (errors.getLength() > 0) ){				
				...
				while(itr.hasNext()){
					tmpType = itr.next();
					//log.debug(tmpType.getSetName());
					//res.put(tmpType.getSetName(), tmpType.getSetSpec());
					res.put(tmpType.getSetSpec(), tmpType.getSetSpec());
				}
			}
		} catch (Exception e) {
			log.error( "OAI ListSets failed", e );
		} 
		sets = null;
		xmlType = null;
		return res;
