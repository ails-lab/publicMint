...
public class ItemDAO extends DAO<Item, Long> {
	...
	public List<Item> getItemsByDataset( Dataset ds, long start, int max ) {
		List<Item> li = getSession().createQuery( " from Item where datasetId=:ds order by item_id ")
			.setEntity("ds", ds)
			.setMaxResults(max)
	...
	public List<Long> getItemIdsByDataset( Dataset ds ) {
		List<Long> res = getSession().createQuery( "select dbID from Item where datasetId=:ds order by item_id ")
				.setEntity("ds", ds)
				.list();		
		return res;
	}
	...
	public List<Item> getValidItemsByDataset( Dataset ds, long start, int max ) {
		List<Item> li = getSession().createQuery( " from Item where datasetId=:ds and valid = true order by item_id ")
			.setEntity("ds", ds)
			.setMaxResults(max)
			.setFirstResult((int) start )
			.list();
	...
	public List<Item> getInvalidItemsByDataset( Dataset ds, long start, int max ) {
		List<Item> li = getSession().createQuery( " from Item where datasetId=:ds and valid = false order by item_id ")
			.setEntity("ds", ds)
			.setMaxResults(max)
			.setFirstResult((int) start )
			.list();
	...
	public Item getDerived( Item item, Dataset ds ) {
		...
		Item result = (Item) getSession().createQuery( " from Item where sourceItem = :item and datasetId = :ds")
		.setEntity("item", item)
		.setEntity( "ds", ds)
		.uniqueResult();
		return result;		
	...
	public List<Object[]> getIdPublished( String id) {
		List<Object[]> published = DB.getSession().createQuery( "select xs.name, ds, pr " +
	            "from PublicationRecord pr, Item i " +
	            "join pr.publishedDataset ds " +
	            " join ds.schema xs " +
				"where  " +
				"i.datasetId = ds " +
				"and pr.status = 'OK' " +
...
*/
	...
	public void applyForDataset( Dataset ds, ApplyI<Item> operation, boolean withState ) throws Exception {
		String cond = "datasetId = " + ds.getDbID() + " order by item_id";
		if( withState )
	...
	/**
	 * Return item and schema name array.
	 * @param id
	 * @return
	 */
	public List<Object[]> getPublishedItems( String id) {
		List<Object[]> published = DB.getSession().createQuery( "select i, xs.name " +
	            "from PublicationRecord pr, Item i " +
	            "join pr.publishedDataset ds " +
	            "join ds.schema xs " +
				"where  " +
	...
	public void changeAllItems( Dataset ds, ApplyI<Item> operation ) throws Exception {
		...
		sr =  getSession().createQuery("from Item where datasetId = :ds order by item_id" )
				.setEntity("ds", ds)
				.setFetchSize(100)
