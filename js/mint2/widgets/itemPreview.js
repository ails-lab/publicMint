...
ItemPreview.prototype.render = function() {
	...
	if(this.settings.compact == false) {
		this.control = $("<div>").addClass("mint2-preview-control").prependTo(this.container);
		...
		if(this.settings.itemId != null) {
			this.previewItem(this.settings.itemId, null, [ "dataset,schema" ]);
		}		
	}
}
...
ItemPreview.prototype.previewItem = function(itemId, mappingId, columns) {
	...
	this.itemPreviews.show();
	if(!this.settings.compact) {
		this.control.css("width", "30%");
		this.itemPreviews.css({
			"margin-left": "10px",
			"width": "66%"
		});
	}
	
	this.firstView.empty().append(Mint2.loading());
...
//		});						
		...
		$("#externalLink").remove();
		var iframe = $("<iframe/>", { "src":view.url, "width":"100%", "height":"900px", "id":"externalLink", "class":"loader"} );
...
(function($) {	
	...
	var methods = {
		...
		refresh: function() {
			...
			this.contents.append($("<div>").addClass("mint2-preview-option").append(Mint2.loading()));
