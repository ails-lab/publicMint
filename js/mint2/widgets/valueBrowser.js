				...
				success: function(response) {
					if(response != undefined) {
						if (response.error != undefined) {
							data.gridContainer.empty();
							data.gridContainer.append(Mint2.message("No such xpath exists in any of the items.", Mint2.WARNING));
						}
						...
						if(settings.showInfo) {
							...
							if(Object.keys(information).length > 0) {
								Mint2.dataTable({
									...
									data: information,
									download: "api/download?xpathHolder="+settings.xpathHolderId
								}).appendTo(data.info);
							}
						}
