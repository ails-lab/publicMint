(function($) {
	...
	var functions = [
	     ...
	     },
	     {
	    	 	...
	    	 	message: "Replace all occurences of the first argument with the second argument.",
	    	 	arguments: [
	    	 	            { label: "replace this: " },
	    	 	            { label: "with this: " }
	    	 	            ]
	     },
	     {
	    	 	...
	    	 	message: "Replace all occurences matching the regular expression" +
	    	 			" (first argument) with the second argument (string).",
	    	 	arguments: [
	    	 	            { label: "replace based on regular expression: " },
	    	 	            { label: "with this: " }
	    	 	            ]
	     },
	     {
	    	 	name: "trim",
	    	 	description: "Trim",
	    	 	message: "Trims leading and trailing whitespaces"
	     },
	     {
	    	 	name: "split",
	    	 	description: "Split",
				...
				switch(calldata.call) {
					...
					case "replaceString":
						if (calldata.arguments != undefined && calldata.arguments.length > 1) {
		 	    			var arg0 = decodeURIComponent(calldata.arguments[0]);
		 	    			var arg1 = decodeURIComponent(calldata.arguments[1]);
		 	    			result = value.split(arg0).join(arg1);
						}
						break;
					case "split":
			 	    		if(calldata.arguments != undefined && calldata.arguments.length > 1) {
