(function($) {
	...
	var defaults = {
		...
		'onDuplicate': function(response, settings) {
			...
			if (original != undefined && newItem != undefined) {
				...
				node.hide().fadeIn();
				settings.editor.validate();
				$("#" + original).find(".mapping-action-duplicate").
					toggleClass("mapping-element-loading");
			}
		},
		'onRemove': function(response, settings) {
			var id = response.id;
			if(id != undefined) {
	...
	function render(container) {
		...
		var isMixed = ( target.mixed != undefined );
...
//			console.log(data.editor);
			...
			// right side actions
			var right_actions = $("<div>").addClass("mapping-element-right-actions").
			appendTo(header);
			if (target.fixed == undefined) {
				if(data.settings.showDuplicate) {
					//TODO: in single annotator, where showRemove=always, remove should not 
					//appear only if the element has a text value, attribute with value, or 
					//child with value. As it is now, showRemove appears in all elements, 
					//even if they appear just because of the schema, i.e., won't be
					//in the output-annotated item.
					if (data.settings.showRemove == "always" || //target.minOccurs < 1 || target.minOccurs == undefined ||
						...
						if (data.editor.annotatorMode == undefined || 
							(data.editor.annotatorMode == XMLAnnotator.MODE_SINGLE && targetHasMappings(target))) {
							var removeDuplicate = $("<div>").attr("title", "Remove duplicate element").
							addClass("mapping-action-remove-duplicate").appendTo(right_actions);
							removeDuplicate.click(function() {
								removeDuplicate.toggleClass("mapping-element-loading");
								if (data.editor.annotatorMode == XMLAnnotator.MODE_SINGLE && 
									data.editor.mode == XMLAnnotator.MODE_SIMPLE && 
									data.settings.showAttributeDuplicate) {
									data.editor.ajax.call("removeNodeAttribute", {id: target.id}, data.settings.onRemove);
								}
								else {
					...
					if (data.editor.annotatorMode == XMLAnnotator.MODE_SINGLE && 
							data.editor.mode == XMLAnnotator.MODE_SIMPLE) {
						//if (target.name.indexOf("@") >= 0) {
							//if (data.settings.showAttributeDuplicate)
								if (data.settings.showAttributeDuplicate) {
									var duplicate = $("<div>").
									...
									duplicate.click(function() {
										...
										data.editor.ajax.call("duplicateNodeAttribute", {id: target.id}, data.settings.onDuplicate);
									});
								}
						//}
					}
			...
			if(!isStructural || isMixed  ) {
				if((mappingsStructural != undefined) && !isMixed)
					mappingsStructural.hide();
