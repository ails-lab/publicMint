...
//				});
		...
		if(data.target.thesaurus != undefined) {
			...
			thesaurus = new ThesaurusBrowser(browser, {
		...
		var numOfCols = outputThesaurus? 3: 2; 
		var columnWidth = (node.innerWidth() - deleteColumnWidth) / numOfCols;
		var columns = [{ id: "input", name: "Input", field: "input", width: columnWidth}];
		if (outputThesaurus)
			columns.push({id: "thesPrefLabel", name: "PrefLabel", field: "thesPrefLabel", width: columnWidth});
		columms = $.merge(columns, 
		...
		grid.onClick.subscribe(function (e, args) {
			var cell = args.cell;
			var activeNode = $(grid.getCellNode(args.row, 0));
			var input = activeNode.text();
       
            if (grid.getColumns()[cell].id == "remove") {
				$.ajax({
					...
					success: function(response) {
						data.list = response["mapping-cases"][data.settings.case].mappings[data.index].valuemap;
		...
		add.click(function () {
			if (thesaurus != undefined) {
				thesLang = thesaurus.getLanguage();
			}
			$.ajax({
				url: data.settings.ajaxUrl,
				context: this,
