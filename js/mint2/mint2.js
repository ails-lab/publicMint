...
// create a div covering everything with some transparency and
// a loading widget in the middle. close it when the underlying call
// has results
// ajaxparam is the object to be passed into $.ajax
Mint2.slowCall = function( ajaxParam ) {
	let wrapParam = {...ajaxParam};
	...
	overlayDiv.style.backgroundColor = 'rgba(255, 255, 255, 0.0)'; // Adjust opacity here (0 - 1)

	const loadingImg = document.createElement('img');
	loadingImg.src = 'images/loader-flower-gray.gif'; // Replace with your gif path
	loadingImg.style.position = 'absolute';
	loadingImg.style.top = '50%';
	loadingImg.style.left = '50%';
	loadingImg.style.transform = 'translate(-50%, -50%)'; // Center the image
	loadingImg.style.width = '10%'; // Adjust width as needed

  	// Append the overlay to the body
	document.body.appendChild(overlayDiv);

	const timeoutId = setTimeout(function() {
	  if (overlayDiv.parentNode) { // Check if overlay is still present
	    overlayDiv.style.opacity = 0.5; // Adjust new opacity here (0 - 1)
		// Append the loading image to the overlay
	  	overlayDiv.appendChild(loadingImg);
	  }
	}, 1000); // 1000 milliseconds = 1 second

	wrapParam.success = function( resp ) {
		document.body.removeChild(overlayDiv);
		clearTimeout(timeoutId);
		ajaxParam.success( resp );
	}

	wrapParam.error = function( resp ) {
		document.body.removeChild(overlayDiv);
		clearTimeout(timeoutId);
		ajaxParam.error( resp );
	}
...
Mint2.infoPopup = function( positionElement, infoText ) {
	const posRect = positionElement.getBoundingClientRect();
	const goLeft = (posRect.left / window.innerWidth) > 0.5;
	const goUp = (posRect.bottom / window.innerHeight) > 0.5;

	const infoDiv = $("<div>")
		.addClass("ui-state-ok mint-message")
	...
	// const infoWidth = infoRect.right-infoRect.left;
	// const infoHeight = infoRect.bottom - infoRect.top;
	
	const top = goUp? posRect.top -20-infoDiv.height():posRect.bottom+20;
	const left = goLeft? posRect.left -20 -infoDiv.width():posRect.left+20;
    ...
    setTimeout( function() {
		const elem = document.querySelector('#infoDiv');
		if( elem ) {
			elem.parentNode.removeChild( elem );
			// console.log( "Would remove popup now");
		}
	}, 2000 );
}

Mint2.modalDialog = function( content, title, callBack ) {
	if(title == undefined) title = "Mint2";
	var $dialog = $('<div>')
	.html(content)
	.dialog({
...
Mint2.modalDialogWithCancel = function( content, title, callBack ) {
	if(title == undefined) title = "Mint2";
	var $dialog = $('<div>')
	.html(content)
	.dialog({
...
Mint2.dataTable = function(options) {
	...
	if( settings.download ) {
		 $("<a>")
			...
			.appendTo( rows )
			.click( ()=>{window.location=settings.download;});
	}
...
Mint2.searchBox = function(options) {
	...
	if( options.selections != undefined ) {
		var input = Mint2.searchBoxInputDefault(options, select, 0);
		input.appendTo(box);
		...
		select.change(function() {
			...
			if (selectedVal == "field") {
				//var xpathField = $("<div>").text("!")
				//.addClass("mapping-function-message").appendTo($("<input>"));
				var initXpathArea = "Field (drag and drop from schema)";
				$("#input-0").remove();
				var xpath = $("<div>")
				.addClass("schema-tree-drop")
				.attr("id", "searchXpath1")
				.addClass("mapping-element-mapping");
				var area = $("<input>")
				.addClass("mapping-element-mapping-area")
				.addClass("defaultTextInput")
				.attr("id", "searchXpath")
				//.attr('contenteditable','true')
				.val(initXpathArea).appendTo(xpath);
				...
				.addClass("mapping-element-mapping-area")
				//.attr('contenteditable','true')
				.val("Value").appendTo(value);*/
				xpath.after(value);
				$(value).click(function(){
					if (value.val() == "Value ") {
						value.removeClass("defaultTextInput")
						value.val("");
					}
				});
				$(value).keydown(function(event) {
					if (event.keyCode == 13) {
						if (area.val() != initXpathArea) {
							var term = {"xpath": area.val(), "value": $(this).val()};
							if (options.callback != undefined) 
								options.callback(term, selectedVal);
						}
					}
				});
			}
				...
				$("#searchValue").remove();
				$("#input-0").remove();
				select.before(Mint2.searchBoxInputDefault(options, select, 0));
			}
		});
	}
	else {
		var input = Mint2.searchBoxInputDefault(options, select, 1);
		input.appendTo(box);
	}
	return container;
}

Mint2.searchBoxInputDefault = function(options, select, flag) {
	var input = $("<input>")
	.attr("id", "input-"+flag)
	...
	if(options.prompt){
		input.attr("placeholder", options.prompt);
	}
	return input;
}
