...
edmTypeAttr = edmType.getAttribute("@lido:type").addConstantMapping("europeana:type");
...
recRightsType.addEnumeration("http://rightsstatements.org/vocab/NoC-OKLR/1.0/");
recRightsType.addEnumeration("http://rightsstatements.org/vocab/NoC-NC/1.0/");
...
recRightsType.addEnumeration("http://rightsstatements.org/vocab/InC/1.0/");
recRightsType.addEnumeration("http://rightsstatements.org/vocab/InC-EDU/1.0/");
recRightsType.addEnumeration("http://rightsstatements.org/vocab/InC-OW-EU/1.0/");
recRightsType.addEnumeration("http://rightsstatements.org/vocab/CNE/1.0/");
...
rightsType.addEnumeration("http://rightsstatements.org/vocab/NoC-OKLR/1.0/");
rightsType.addEnumeration("http://rightsstatements.org/vocab/NoC-NC/1.0/");
...
rightsType.addEnumeration("http://rightsstatements.org/vocab/InC-EDU/1.0/");
rightsType.addEnumeration("http://rightsstatements.org/vocab/InC-OW-EU/1.0/");
rightsType.addEnumeration("http://rightsstatements.org/vocab/CNE/1.0/");
rightsTypeEuropeana = rightsType;

// 	Set image thumb and image master
thumbiiif = cache.duplicate(template.findFirst("//lido:administrativeMetadata/lido:resourceWrap/lido:resourceSet/lido:resourceRepresentation").getId());
thumbiiif.setLabel("resourceRepresentation (iiif_image)");
thumbiiif.setRemovable(true);
...
linkResource = thumbiiif.getChild("lido:linkResource");
linkResource.setLabel("linkResource (iiif_image_api)");
linkType = thumbiiif.getAttribute("@lido:type");
linkType.addConstantMapping("iiif_image_api");
linkType.setFixed(true);
linkResourceThumb = linkResource;
...
recordSourceb = cache.duplicate(template.findFirst("//lido:administrativeMetadata/lido:recordWrap/lido:recordSource").getId());
...
recordSourceb.setLabel("recordSource (based on Europeana)");
recordSourceTypeb = recordSourceb.getAttribute("@lido:type").addConstantMapping("europeana:intermediateProvider").setFixed(true)

originalRecordSource.setString(JSONMappingHandler.ELEMENT_MINOCCURS, "0");

//Add the europeana data intermediate provider 
//originalRecordSource = template.findFirst("//lido:recordSource");
//recordSourceb = cache.duplicate(template.findFirst("//lido:administrativeMetadata/lido:recordWrap/lido:recordSource").getId());
//recordSource.setLabel("recordSource (based on europeana)");
//recordSourceType = recordSource.getAttribute("@lido:type").addConstantMapping("europeana:intermediateProvider").setFixed(true)
//recordSourceAppellation = recordSource.getChild("lido:legalBodyName").setMandatory(false);
//originalRecordSource.setString(JSONMappingHandler.ELEMENT_MINOCCURS, "0");
...
mappings.addBookmark(" 4. Thematic Context (R)", thematicContext);
...
mappings.addBookmark(" 8. Current Repository - Location (M/A)",  repositoryLocation);
mappings.addBookmark(" 8a. Current Repository - Location Identifier (if TGN URI exists M)",  repositoryLocationPlaceID);
mappings.addBookmark(" 8b. Current Repository - Location is part of TGN Nation (if TGN URI exists M/A)",  repositoryLocationPartOfPlace);  
